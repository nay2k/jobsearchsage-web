...
// node_modules/@unovis/ts/types/data.js
var FindNearestDirection;
(function(FindNearestDirection2) {
  FindNearestDirection2["Left"] = "left";
  FindNearestDirection2["Right"] = "right";
  FindNearestDirection2["Auto"] = "auto";
})(FindNearestDirection || (FindNearestDirection = {}));

// node_modules/@unovis/ts/utils/data.js
...
var isAClassInstance = (a) => a.constructor.name !== "Function" && a.constructor.name !== "Object";
var isPlainObject = (a) => isObject(a) && !isArray(a) && !isFunction(a) && !isAClassInstance(a);
var isEmpty = (obj) => {
  return [Object, Array].includes((obj || {}).constructor) && !Object.entries(obj || {}).length;
};
var isEqual = (a, b, skipKeys = [], visited = /* @__PURE__ */ new Set()) => {
  if (Array.isArray(a)) {
...
var without = (arr, ...args) => arr.filter((item) => !args.includes(item));
var flatten2 = (arr) => arr.flat();
var cloneDeep = (obj, stack = /* @__PURE__ */ new Map()) => {
...
var merge2 = (obj1, obj2, visited = /* @__PURE__ */ new Map()) => {
  if (!obj1 || !obj2)
    return obj1;
  if (obj1 === obj2)
    return obj1;
  ...
  Object.keys(obj2).forEach((key) => {
    ...
    if (isPlainObject(obj1[key]) && isPlainObject(obj2[key])) {
      newObj[key] = merge2(obj1[key], obj2[key], visited);
...
var groupBy = (arr, accessor) => {
  return arr.reduce((grouped, v, i, a, k = accessor(v, i)) => ((grouped[k] || (grouped[k] = [])).push(v), grouped), {});
};
var sortBy = (arr, accessor) => {
  return arr.concat().sort((a, b) => {
    return accessor(a) > accessor(b) ? 1 : accessor(b) > accessor(a) ? -1 : 0;
  });
};
var throttle2 = (f, delay, options) => throttle(delay, f, options);
function getValue(d, accessor, index2) {
  if (isFunction(accessor))
    return accessor(d, index2);
  else
    return accessor;
}
...
function clamp(d, min2, max2) {
  return Math.min(Math.max(d, min2), max2);
}
...
function getStackedValues(d, index2, ...acs) {
  const values = [];
  let positiveStack = 0;
  let negativeStack = 0;
  for (const a of acs) {
    const value = getNumber(d, a, index2) || 0;
    if (value >= 0) {
...
function getStackedData(data, baseline, acs, prevNegative) {
...
function getNearest(data, value, accessor, direction = FindNearestDirection.Auto) {
  ...
  const dataWithIndexSorted = dataWithIndex.sort(([a, i], [b, j]) => getNumber(a, accessor, i) - getNumber(b, accessor, j));
  const values = dataWithIndexSorted.map(([d, i]) => getNumber(d, accessor, i));
  const index2 = direction === FindNearestDirection.Right ? bisectLeft(values, value, 0, data.length - 1) : bisectRight(values, value, 1, data.length);
  if (direction === FindNearestDirection.Right) {
    return dataWithIndexSorted[index2][0];
  } else if (direction === FindNearestDirection.Left) {
    return dataWithIndexSorted[index2 - 1][0];
  }
  return value - values[index2 - 1] > values[index2] - value ? dataWithIndexSorted[index2][0] : dataWithIndexSorted[index2 - 1][0];
}
function filterDataByRange(data, range2, accessor, includeNeighbors = false) {
  if (!accessor)
    return [];
  ...
  if (includeNeighbors) {
    if (filteredData.length === 0) {
      const nearestLeft = getNearest(data, range2[0], accessor, FindNearestDirection.Left);
      const nearestRight = getNearest(data, range2[1], accessor, FindNearestDirection.Right);
...
function isNumberWithinRange(value, range2) {
  return value >= range2[0] && value <= range2[1] || value >= range2[1] && value <= range2[0];
}
...
function escaping(index2, count2) {
  while (--count2 && next())
    if (character < 48 || character > 102 || character > 57 && character < 65 || character > 70 && character < 97)
      break;
  return slice2(index2, caret() + (count2 < 6 && peek() == 32 && next() == 32));
}
...
var createEmotion = function createEmotion2(options) {
  ...
  var cx2 = function cx3() {
    ...
    return merge3(cache2.registered, css2, classnames(args));
  };
  return {
    css: css2,
    cx: cx2,
    ...
    merge: merge3.bind(null, cache2.registered, css2)
  };
};
...
//# sourceMappingURL=chunk-7S7APBZD.js.map
